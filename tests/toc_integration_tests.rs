...
fn test_basic_toc_generation() {
    ...
    let result2 = process_toc(&expected);
    assert_eq!(
        result2.output.trim(),
        expected.trim(),
...
fn test_code_blocks() {
    ...
    assert!(!result.output.contains("[Example Header]"));
    assert!(!result.output.contains("[Should not appear in TOC]"));
    assert!(!result.output.contains("[Neither should this]"));
}
...
fn test_multiple_code_blocks() {
    ...
    assert!(result.output.contains("[Section 1]"));
    assert!(result.output.contains("[Section 2]"));
    assert!(result.output.contains("[Section 3]"));
    assert!(result.output.contains("[Section 4]"));

    // Verify fake headers are NOT in TOC
    assert!(!result.output.contains("[Fake header 1]"));
    assert!(!result.output.contains("[Another fake header]"));
    assert!(!result.output.contains("[Not real]"));
    assert!(!result.output.contains("[Also fake]"));
}
...
fn test_skip_code_blocks() {
    ...
    assert!(result.output.contains("[Real Section 1]"));
    assert!(result.output.contains("[Real Section 2]"));
    assert!(result.output.contains("[Real Subsection]"));

    // Verify fake headers in code blocks are NOT in TOC
    assert!(!result.output.contains("[Fake Header in Code Block]"));
    assert!(!result.output.contains("[Another Fake Header]"));
